...
export class shoppingMarket {
    ...
    addProduct(product: shoppingProduct){
        ...
        if(this.products.includes(product)){
            console.log("You have added duplicate products to same market: " + product.name);
            return;
        }
        else{
            this.products.forEach(element => {
                if(product.name == element.name){
                    console.log("You have added duplicate products to same market, not possible: " + product.name + " , " + this.name);
                    shouldReturn = true;
                    return;
                }
            });
